...
class EstimateOneAPIScraper:
    ...
    def insert_to_supabase(self, project_data: Dict[str, Any]) -> bool:
        ...
        try:
            logger.debug(f"Inserting project '{project_data.get('Project Name', 'Unknown')}' to database")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i, row in enumerate(project_rows, 1):
                logger.debug(f"Processing project {i}/{len(project_rows)}")
                ...
                if project_data:
                    ...
                    if scraper.insert_to_supabase(project_data):
                        ...
                        logger.debug(f"Processed: {project_data.get('Project Name', 'Unknown')}")
...
def _scrape_projects_by_ids_sync(
    ...
    """Process multiple project IDs using search functionality with ID deletion after successful processing"""
    logger.info(f"Starting project ID processing for {len(project_ids)} projects")
    
    results = {"processed": 0, "failed": 0, "details": [], "sample_project": {}}
    successfully_processed_ids = []  # Track successfully processed IDs for deletion
            ...
            for i, project_id in enumerate(project_ids, 1):
                try:
                    ...
                    if not scraper.search_project_by_id(page, project_id):
                        logger.warning(f"Project {project_id} not found in search")
                        results["failed"] += 1
                    ...
                    try:
                        ...
                        logger.debug(f"Popup may not have opened for project {project_id}, continuing anyway...")
                    ...
                    if scraper.insert_to_supabase(project_data):
                        results["processed"] += 1
                        successfully_processed_ids.append(project_id)  # Track for deletion
                        ...
                        logger.warning(f"Failed to save project {project_id} to database")
    ...
    # Add successfully processed IDs to results for frontend deletion
    results["successfully_processed_ids"] = successfully_processed_ids
    
    logger.info(f"Finished processing {len(project_ids)} project IDs. Results: {results['processed']} processed, {results['failed']} failed")
    logger.info(f"Successfully processed IDs for deletion: {successfully_processed_ids}")
...
async def scrape_estimate_one(req: EstimateOneRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Successfully decrypted credentials for user: {user_id}")
        except Exception as e:
...
async def scrape_projects_by_ids(
    ...
    Handles both single and multiple project IDs with ID deletion after successful processing
    """
    ...
    try:
        ...
        # Handle successful ID deletion - these IDs were successfully processed
        ...
        response_data = {
            ...
            "successfully_processed_ids": successfully_processed_ids,  # Include for frontend cleanup
            "scraped_at": datetime.utcnow().isoformat(),
